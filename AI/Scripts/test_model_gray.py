...
interpreter = tf.lite.Interpreter(model_path="model_gray.tflite")
...
class_names = ['bright', 'soft', 'generic']

def extract_mel_spectrogram(file_path, duration=4.0, sr=22050):
    audio, _ = librosa.load(file_path, sr=sr, duration=duration)
    mel_spec = librosa.feature.melspectrogram(y=audio, sr=sr, n_mels=252, fmax=8000)
    mel_spec_db = librosa.power_to_db(mel_spec, ref=np.max)
    mel_spec_img = Image.fromarray(mel_spec_db).resize((252, 252))
...
file_path = 'WAV/bright/sample_85.wav'
...
plt.title('MEL Spectrogram (252x252)')
plt.axis('off')
plt.show()
